...
flags.DEFINE_string(
    "data_examples", None,
    "The input file for tf.examples. If doesn't exist, it will be created.")
...
flags.DEFINE_string(
    "cdc_spm_model_file", None,
    "The model file for sentence piece used for cdc specifically.")

flags.DEFINE_integer(
    "cdc_max_seq_length", 128,
    "The maximum total input sequence length after WordPiece tokenization. "
    "Sequences longer than this will be truncated, and sequences shorter "
    "than this will be padded.")

flags.DEFINE_integer("cdc_vocab_size", 500, "Vocabulary length")

flags.DEFINE_integer("cdc_embedding_size", 32, "Lenght of the embedding")

flags.DEFINE_bool("cdc_only", False, "Only train the cdc thing")

flags.DEFINE_string(
    "cdc_init_checkpoint", None,
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, age, sex, label=None):
...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.input_cdc_ids = input_cdc_ids
        self.age = age
...
class MnliProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            text_a = tokenization.preprocess_text(line[8],
                                                  lower=FLAGS.do_lower_case)
            text_b = tokenization.preprocess_text(line[9],
                                                  lower=FLAGS.do_lower_case)
            if set_type == "test":
                label = "contradiction"
            else:
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            if i == 0:
                continue
            guid = "%s-%s" % (set_type, i)
            text_a = tokenization.preprocess_text(line[3],
                                                  lower=FLAGS.do_lower_case)
            text_b = tokenization.preprocess_text(line[4],
                                                  lower=FLAGS.do_lower_case)
            if set_type == "test":
                guid = line[0]
                label = "0"
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if set_type == "test":
                ...
                label = "0"
                age, sex = int(line[2]), int(line[3])
            else:
                text_a = tokenization.preprocess_text(
                    line[3], lower=FLAGS.do_lower_case)
                label = tokenization.preprocess_text(line[1])
                age, sex = int(line[4]), int(line[5])
            examples.append(
                InputExample(guid=guid,
                             text_a=text_a,
                             age=age,
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    if isinstance(example, PaddingInputExample):
        return InputFeatures(input_ids=[0] * max_seq_length,
                             ...
                             input_cdc_ids=[0] * FLAGS.cdc_max_seq_length,
                             age=0.0,
                             sex_ids=[0, 0],
                             label_id=0,
    ...
    input_cdc_ids = tokenizer.sp_cdc.encode_as_ids(example.text_a)
    while len(input_cdc_ids) < FLAGS.cdc_max_seq_length:
        input_cdc_ids.append(0)

    assert len(input_cdc_ids) == FLAGS.cdc_max_seq_length

    # One hot encode the sex.
    if example.sex == 1:
        sex_ids = [1.0, 0.0]
    else:
        assert example.sex == 2
        sex_ids = [0.0, 1.0]
    ...
    if ex_index < 5:
        ...
        tf.logging.info("input_cdc_ids: %s" %
                        " ".join([str(x) for x in input_cdc_ids]))
        tf.logging.info("age: %s" % (str(age)))
        tf.logging.info("sex_ids : %s" % " ".join([str(x) for x in sex_ids]))
        tf.logging.info("label: %s (id = %d)" % (example.label, label_id))

    feature = InputFeatures(input_ids=input_ids,
                            input_mask=input_mask,
                            segment_ids=segment_ids,
                            input_cdc_ids=input_cdc_ids,
                            age=age,
                            sex_ids=sex_ids,
                            label_id=label_id,
                            is_real_example=True)
...
def file_based_convert_examples_to_features(examples, label_list,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features["input_cdc_ids"] = create_int_feature(feature.input_cdc_ids)
        features["age"] = create_float_feature([feature.age])
        features["sex_ids"] = create_float_feature(feature.sex_ids)
        features["input_cdc_ids"] = create_int_feature(feature.input_cdc_ids)
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
    ...
    name_to_features = {
        ...
        "input_cdc_ids": tf.FixedLenFeature([FLAGS.cdc_max_seq_length],
                                            tf.int64),
        "age": tf.FixedLenFeature([1], tf.float32),
        "sex_ids": tf.FixedLenFeature([2], tf.float32),
...
def create_model(albert_config, is_training, input_ids, input_mask,
    ...
    with tf.variable_scope('cdc'):
        ...
        with tf.variable_scope("dense_1"):
            input_size = concat_max_average.shape[-1].value
            ...
            dense_1 = tf.matmul(concat_max_average, W)
            dense_1 = tf.nn.bias_add(dense_1, b)
            dense_1 = tf.nn.relu(dense_1)

        output_cdc_layer = tf.concat([age, sex_ids, dense_1], axis=-1)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, predictions,
                ...
                loss = tf.metrics.mean(values=per_example_loss,
                                       weights=is_real_example)
                mean_per_class_accuracy = tf.metrics.mean_per_class_accuracy(labels=label_ids, 
                        predictions=predictions, num_classes=num_labels, weights=is_real_example)
