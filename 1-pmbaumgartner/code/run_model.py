...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        pd.DataFrame(preds, columns=["label"]).assign(
            event=lambda x: x["label"].map(dict(enumerate(LABEL_LIST)))
        ).to_csv(os.path.join(eval_output_dir, prefix, "preds.csv"))
        pd.DataFrame(plogits, columns=LABEL_LIST).to_csv(
            os.path.join(eval_output_dir, prefix, "plogits.csv")
        )
...
def main():
    ...
    parser.add_argument(
        "--pred_output_dir", type=str, default="", help="Predictions output dir"
    )
