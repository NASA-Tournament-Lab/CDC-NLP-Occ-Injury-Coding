...
class ModifiedColaProcessor(processors['cola']):
    def get_labels(self):
        """See base class."""
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_t_examples(
            self._read_tsv(os.path.join(data_dir, 'test.tsv')), 'test')
    
    def _create_t_examples(self, lines, set_type):
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    processor = ModifiedColaProcessor()
...
def main():
    ...
    if args.do_predict:
        ...
        model.to(args.device)
        processor = ModifiedColaProcessor()
        features = convert_examples_to_features(processor.get_test_examples(args.data_dir),
                                                tokenizer,
                                                label_list=processor.get_labels(),
                                                max_length=args.max_seq_length,
        ...
        # Eval!
        logger.info("***** Running evaluation {} *****".format('a'))
        logger.info("  Num examples = %d", len(eval_dataset))
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                hidden_states = hidden_states[1].sum(dim=1)
...
#             preds = np.argmax(preds, axis=1)
        ...
        preds.to_csv(args.output_dir + 'result.csv', index=False)
